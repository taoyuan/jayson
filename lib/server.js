...
/**
 *  Constructor for a RJ Server
 *  @class Server
 *  @extends require('events').EventEmitter
 *  @param {Object} [methods] Methods to add
 *  @param {Object} [options]
 *  @param {Boolean} [options.collect=false] Passed to RJ.Method as an option when created
 *  @param {Boolean} [options.params] Passed to RJ.Method as an option when created
...
Server.prototype.method = function (name, definition, scope) {
  ...
  var isRelay = definition instanceof rj.Client;
  var isMethod = definition instanceof rj.Method;
  ...
  if (!isRelay && !isMethod && !isFunction) {
    throw new TypeError('method definition must be either a function, an instance of rj.Client or an instance of rj.Method');
  }
  ...
  // make instance of rj.Method
  if (!isRelay && !isMethod) {
    definition = new rj.Method(definition, {
...
Server.prototype.call = function (request, originalCallback) {
  ...
  maybeParse(request, this.options, function (err, request) {
    ...
    if (method instanceof rj.Client) {
    ...
    if (!(method instanceof rj.Method)) {
      return respond(self.error(Server.errors.METHOD_NOT_FOUND));
    }

    // execute rj.Method instance
...
Server.prototype._resolveRouter = function (method, params) {
  ...
  // got a rj.Method or a rj.Client, return it
  if ((resolved instanceof rj.Method) || (resolved instanceof rj.Client)) {
    return resolved;
  }

  // got a regular function, make it an instance of rj.Method
  if (_.isFunction(resolved)) {
    return new rj.Method(resolved);
  }
